...
from mlopskit.utils.logging import ContextualizedLogging
    ...
    def __init__(
        ...
        **driver_settings,
        ...
        self.timeout = timeout_s or int(
            os.environ.get("MLOPSKIT_STORAGE_TIMEOUT_S", 300)
        )
        self.prefix = (
            prefix or os.environ.get("MLOPSKIT_STORAGE_PREFIX") or "mlopskit-assets"
        )
        self.force_download = force_download or bool(
            os.environ.get("MLOPSKIT_STORAGE_FORCE_DOWNLOAD")
        )

        provider = provider or os.environ.get("MLOPSKIT_STORAGE_PROVIDER")
        if not provider:
